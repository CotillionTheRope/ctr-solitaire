...
public class UserInput : MonoBehaviour
{
  ...
  void GetMouseClick() {
    if(Input.GetMouseButtonDown(0)) {
      ...
      clickCount++;
      Vector3 moutPosition = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, -10));
  ...
  void Card(GameObject selected) {
    ...
    if(!selected.GetComponent<Selectable>().faceUp) { //if the card clicked on is face down
      ...
      //else if there is already a card selected and it is the same card
        //if the time is short enough then it is a doulbe click
  ...
  bool Stackable(GameObject selected) {
    ...
    if(!s2.inDeckPile) {
      ...
      //if in the bottom pile must stack alernate colors King to Ace
      else {
        if(s1.value == s2.value - 1) {
          bool card1Red = true;
          bool card2Red = true;
  ...
  void Stack(GameObject selected) {
    //if on top of kind or empty bottom stack the cards in place
    ...
    slot1.transform.parent = selected.transform; //this make the children move with the parents
    ...
    for (int i = 0; i < solitaire.topPos.Length; i++) {
      ...
      if(selected.GetComponent<Selectable>().value == 1) { //if it is an Ace
        ...
        if((solitaire.topPos[i].GetComponent<Selectable>().suit == slot1.GetComponent<Selectable>().suit) && (solitaire.topPos[i].GetComponent<Selectable>().value == slot1.GetComponent<Selectable>().value - 1)) {
          //if it is the last card (has not children)
          if(HasNoChildren(slot1)) {
            slot1 = selected;
            //find a top spot that matches the condtions for auto stacking if it exists
